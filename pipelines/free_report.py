"""FROZEN SNAPSHOT — BAGENT #1 pipeline — report + content optimization.

This is a frozen byte-compatible mirror of pipelines/report.py as of commit
47220b7 (2026-04-08). Its purpose is to provide a stable, unchanging
implementation of the "diagnose + draft" audit pipeline that can power a
future free tier without being disturbed by the ongoing evolution of
pipelines/report.py (see docs/plans/2026-04-08-unified-report-pipeline.md).

DO NOT MODIFY this file. If you need to fix a bug in the main report pipeline,
fix it in pipelines/report.py — the free tier accepts that its output is
frozen at the time it was created. If the frozen output becomes stale enough
to be a problem, take a new snapshot rather than mutating this one.
...
Public entry point: `run_free_report_pipeline(scraped_data, audit_id, on_progress)`
— identical signature to `pipelines.report.run_audit_pipeline` at the time of
snapshot. Internal helpers (prefixed `_`) remain unchanged.
"""
...
async def run_free_report_pipeline(
    scraped_data: Any,
    audit_id: str,
    on_progress: ProgressCallback | None = None,
) -> dict[str, Any]:
    """Frozen BAGENT #1 audit analysis pipeline (10 steps).

    Behavior identical to pipelines.report.run_audit_pipeline as of commit
    47220b7. Do not modify — see module docstring for rationale.
    """
